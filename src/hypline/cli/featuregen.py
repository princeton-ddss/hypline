...
@app.command(name="spectral")
def generate_spectral_feature(
    bids_root: Annotated[
        Path,
        typer.Argument(
            help="BIDS dataset root (contains stimuli/, features/, derivatives/)",
    ...
    """Generate a Whisper log-Mel spectrogram aligned to the BOLD TR grid."""
    from hypline.features.spectral import (
        SpectralFeature,
    ...
    # Discover first: SpectralFeature.__init__ downloads the extractor,
    # so skip constructing it when there is nothing to generate
    _dyad_ids = _dyad_ids or BIDSLayout(bids_root).list.dyads(area="stimuli")
    if not _dyad_ids:
        logger.warning("No dyads found — nothing to generate")
    ...
    config = WhisperFeatureExtractorConfig(model=model, model_dir=model_dir)
