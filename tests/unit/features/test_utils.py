...
class TestDownsampleFeature:
    ...
    def test_downsample_mean_multiple_events_per_tr(self):
        ...
        result = downsample_feature(df, n_trs=2, repetition_time=tr, method="mean")
    ...
    def test_output_dtype_is_float64(self):
        ...
        result = downsample_feature(df, n_trs=2, repetition_time=2.0, method="mean")
    ...
    def test_invalid_method_raises(self):
        ...
        with pytest.raises(ValueError):
            downsample_feature(df, n_trs=1, repetition_time=2.0, method="invalid")
    ...
    def test_single_tr_pass_through(self):
        ...
        tr = 2.0
        df = _make_df([0.0], [[1.0, 2.0]])
        result = downsample_feature(df, n_trs=1, repetition_time=tr, method="mean")
        assert np.allclose(result[0], [1.0, 2.0])

    def test_n_trs_zero_raises(self):
        df = _make_df([0.0, 2.0], [[1.0], [2.0]])
        with pytest.raises(ValueError, match="n_trs must be positive"):
            downsample_feature(df, n_trs=0, repetition_time=2.0, method="mean")
    ...
    def test_empty_input_returns_zero_filled_output(self):
        ...
        result = downsample_feature(df, n_trs=3, repetition_time=2.0, method="mean")
